...
这是一个基于 `edge-extension-template` 自动化模板开发的 Microsoft Edge Manifest V3 扩展。点击工具栏中的扩展图标会打开 Popup，提供四个操作：

- “选择元素”：在当前网页中选择一个元素，把清理后的 Outer HTML 复制到剪贴板。
- “选择元素+”：选择一个元素，把完整元素上下文复制到剪贴板。
- “复制控制台日志”：把当前网页捕获到的 Console 输出复制到剪贴板，最多保留 200 条。
- “截图并编辑”：快捷键触发后立即截取当前可视区域，再在原页面上打开截图编辑浮层调整裁剪区域并下载 PNG；默认快捷键是 `Ctrl+Shift+S`，可在 `edge://extensions/shortcuts` 修改。
...
│  ├─ background/service-worker.js # 日志、快捷键和截图下载
...
- 安装扩展后打开普通网页，点击扩展图标确认 Popup 显示四个操作。
...
- 让网页显示悬停提示或临时气泡，按 `Ctrl+Shift+S`，确认原页面不跳转、页内出现编辑浮层，再拖拽裁剪并下载 PNG；检查截图中悬停图标或临时气泡仍然存在。
