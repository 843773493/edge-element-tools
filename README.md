...
- “选择元素”：在当前网页中选择一个元素，立即把元素信息 JSON 复制到剪贴板。
- “选择元素+”：继续选择元素，把累计结果复制到剪贴板。
- “复制日志”：把本次扩展会话记录的选择日志复制到剪贴板。
...
`npm run e2e` 不会操作你日常使用的 Edge，也不会改动现有浏览器配置。它会启动 Playwright 自带的隔离 Chromium 临时用户目录，后台加载本扩展，打开测试页和 Popup，完成选择元素、选择元素+、剪贴板和复制日志交互后自动关闭并清理临时数据。需要看到浏览器窗口时，可使用 `EDGE_E2E_HEADFUL=1 npm run e2e`。
...
- 点击“选择元素”，在页面上移动并点击目标元素，检查剪贴板中的 JSON。
- 再次点击“选择元素+”，选择另一个元素，检查剪贴板中是累计数组。
- 点击“复制日志”，检查剪贴板中包含 `element_selected` 日志记录。
